  ...
  <main class="main" id="top">
    <div class="container py-4">
      <div class="d-flex align-items-center justify-content-between mb-4">
        <h2 class="mb-0">📌 Abonnements expirés</h2>
...
export default {
  ...
  methods: {
    ...
    async fetchExpires(page = 1) {
      try {
        const { data } = await api.get("/admin/abonnements/expire", {
        ...
        console.error("Erreur chargement expirés :", err.response?.data || err.message);
      }
    },
    async relancer(id) {
      if (!confirm("Voulez-vous relancer ce transporteur par email ?")) return;

      try {
        await api.post(`/admin/abonnements/${id}/relancer`);
        alert("✅ Email de relance envoyé !");
      } catch (err) {
        console.error("Erreur relance :", err.response?.data || err.message);
      }
    },
    changePage(page) {
      if (page > 0 && page <= this.pagination.last_page) {
        this.fetchExpires(page);
      }
    },
